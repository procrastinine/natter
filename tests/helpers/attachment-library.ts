...
type AttachmentRecord = ProcessAttachmentResult['attachment']

interface AttachmentSearchQuery {
  query?: string
  filters?: {
    kind?: AttachmentRecord['kind']
    mime?: string
    origin?: AttachmentRecord['origin']
    storageState?: AttachmentRecord['storageState']
    minSizeBytes?: number
    maxSizeBytes?: number
    minRefCount?: number
    maxRefCount?: number
  }
  sort?: 'created-desc' | 'created-asc' | 'size-desc' | 'size-asc'
  limit?: number
}
...
interface MessageAttachmentRef {
  ...
  kindSnapshot: AttachmentRecord['kind']
  storageStateSnapshot: AttachmentRecord['storageState']
  createdAt: number
  updatedAt: number
}
