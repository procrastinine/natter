...
describe('image-allowlist defaults', () => {
  it('allows built-in https origins exactly', () => {
    expect(
      isImageOriginAllowed('https://openrouter.ai/logo.png', DEFAULT_IMAGE_ORIGINS),
    ).toBe(true)
  })
  ...
  it('allows subdomains under wildcard entries', () => {
    expect(
      isImageOriginAllowed(
        'https://cdn.huggingface.co/pic.png',
        DEFAULT_IMAGE_ORIGINS,
      ),
    ).toBe(true)
    expect(
      isImageOriginAllowed(
        'https://files.openrouter.ai/pic.png',
        DEFAULT_IMAGE_ORIGINS,
      ),
    ).toBe(true)
  })
  ...
  it('accepts data: URIs and blob: URLs', () => {
    ...
    expect(
      isImageOriginAllowed(
        'blob:https://openrouter.ai/abc-123',
        DEFAULT_IMAGE_ORIGINS,
      ),
    ).toBe(true)
  })
  ...
  it('blocks other origins', () => {
    expect(
      isImageOriginAllowed('https://evil.example.com/pixel.gif', DEFAULT_IMAGE_ORIGINS),
    ).toBe(false)
    expect(
      isImageOriginAllowed('http://openrouter.ai/insecure.png', DEFAULT_IMAGE_ORIGINS),
    ).toBe(false)
  })
  ...
  it('respects user-added origins appended to the list', () => {
    ...
    expect(
      isImageOriginAllowed('https://cdn.mysite.example/one.png', augmented),
    ).toBe(true)
  })
})
