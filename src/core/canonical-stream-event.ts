import type {
  ...
  ChatUsageV2,
  CitationFileIdentityV2,
  ContentAnnotationSourceV2,
  ContentAnnotationV2,
  ContentItemV2,
  ...
  ResponsesOutputItemV2,
  ResultSnapshotOutcomeV2,
  ResultSnapshotPayloadV2,
  ResultSnapshotTextPartV2,
  ResultSnapshotToolCallV2,
} from './generation-stream-events'
import { isReasoningEnvelope, isReasoningEnvelopeMutation } from './reasoning-envelope'

type CanonicalLane = CanonicalStreamEventV2['lane']
type CanonicalEventFor<Lane extends CanonicalLane> = Extract<CanonicalStreamEventV2, { lane: Lane }>
...
export function canonicalStreamEventV2FromUnknown(value: unknown): CanonicalStreamEventV2 | null {
  if (
    !isRecord(value) ||
  ...
  return validate(value) ? (value as CanonicalStreamEventV2) : null
}
...
function isReasoningEvent(
  ...
  return (
    ...
    value.mutations.every(isReasoningEnvelopeMutation) &&
...
function isResultSnapshotPayload(value: unknown): value is ResultSnapshotPayloadV2 {
  ...
  return (
    ...
    isReasoningEnvelope(value.reasoningEnvelope) &&
...
function isResultSnapshotTextPart(value: unknown): value is ResultSnapshotTextPartV2 {
  return (
    isRecord(value) &&
...
function isResultSnapshotToolCall(value: unknown): value is ResultSnapshotToolCallV2 {
  return (
    isRecord(value) &&
...
function isResultSnapshotOutcome(value: unknown): value is ResultSnapshotOutcomeV2 {
  if (!isRecord(value) || typeof value.kind !== 'string') return false
...
function isGenerationFailure(value: unknown): value is GenerationStreamFailureV2 {
  return (
    isRecord(value) &&
    hasOnlyKeys(value, [
      'name',
...
function isIntegrity(value: unknown): value is GenerationStreamIntegrityV2 {
  return (
    isRecord(value) &&
    hasOnlyKeys(value, [
      'category',
...
function isServerToolRecord(value: unknown): value is GenerationServerToolCallV2 {
  return (
    isRecord(value) &&
...
function isProviderOutputItem(value: unknown): value is ProviderOutputItemV2 {
  return (
    isRecord(value) &&
    hasOnlyKeys(value, [
      'dialect',
...
function isResponsesOutputItem(value: unknown): value is ResponsesOutputItemV2 {
  return isRecord(value) && typeof value.type === 'string'
}

function isContentItem(value: unknown): value is ContentItemV2 {
...
function isCitationFileIdentity(value: unknown): value is CitationFileIdentityV2 {
  if (!isRecord(value) || typeof value.kind !== 'string') return false
...
function isContentAnnotationSource(value: unknown): value is ContentAnnotationSourceV2 {
  return CONTENT_ANNOTATION_SOURCES.has(value as string)
}

function isUsage(value: unknown): value is Partial<ChatUsageV2> & Record<string, unknown> {
