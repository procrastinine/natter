...
// every char-count cast through `.length`, every content iteration — they all
// can blow up on NaN / undefined / corrupt rows / pathological inputs. This
// module funnels all of that into a handful of small helpers so the counting
// core (tokens.ts / prompt-size.ts / context-cutoff.ts / cost.ts) never has to
// think about it. See `plan/token-counting-audit.md` for the full rationale
// and the audit findings each helper addresses.
//
// Discipline:
...
//     attempt (e.g. 10MB encrypted blob → 3.3M token estimate) or a bug we
//     should not propagate into the UI gauge / budget math.

import type { ContentItem } from './types'

// 100M — order of magnitude above any real model's context window (current
...
// NaN / Infinity / negatives by returning 0 — they'd otherwise poison sums.
