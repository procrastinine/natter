...
import { snapshotMessages, applyStructuralSnapshot } from '../../core/undo'
import {
  BranchIcon,
  CopyIcon,
  InfoIcon,
    ...
    <div data-ui="message-actions" data-mode="default">
      ...
      {isAssistant && onRegenerate ? (
        <button
          ...
          title={
            hasConnection ? 'Regenerate (⇧⌘R)' : disabledTitle
          }
        >
          <ReloadIcon size={14} />
        </button>
      ...
      {isAssistant && onContinue ? (
        <button
          ...
          disabled={!hasConnection}
          aria-label={
            abortReason ? 'Continue partial response' : 'Continue from here'
          }
          title={
            hasConnection
              ? abortReason
